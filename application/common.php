...
use app\common\model\UserCommissionFlow as mUCF;
...
function writeLog($dir,$msg){
    $path = Env::get('runtime_path').$dir."/";
    if (!file_exists ( $path )) {
        mkdir ( "$path", 0777, true );
    }
    $fpath = $path.date("YmdH").".txt";
    $mf = fopen($fpath,'a');
    fwrite($mf, $msg);
    fclose($mf);
}

function createWxacode($imgname,$data){
    $path = __DIR__."/../public/wxacode/";
    if (!file_exists ( $path )) {
        mkdir ( "$path", 0777, true );
    }
    file_put_contents($path.$imgname, $data);
    return '/wxacode/'.$imgname;
}